...
{
	// *************************************************************************
	private	String 	HTMLUNIT_TEST_NAME = "html/parsing/runat/007";	// <-- used in URL; specific to THIS test
	// *************************************************************************
    ...
    private class WorkerThread extends TestRunnable 
    {
        ...
        public void runTest() throws Throwable 
        {
			...
			for (int i=0; i < JaxerHtmlTestUtil.getRequestCount(); i++)
			{
				...
				// Make sure the <span> that was added by server-side script is present in the document
				HtmlElement element = page.getHtmlElementById("span1");
			    assertNotNull( element);

				// Make sure the <script> with runat=server_only is NOT present in the document
				element = null;
				try {
					element = page.getHtmlElementById("s");
				}
				catch (Exception e)	{}
			    assertNull( element);
