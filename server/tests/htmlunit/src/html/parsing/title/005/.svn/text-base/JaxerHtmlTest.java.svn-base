...
{
	// *************************************************************************
	private	String 	HTMLUNIT_TEST_NAME = "html/parsing/title/005";	// <-- used in URL; specific to THIS test
	// *************************************************************************
    ...
    private class WorkerThread extends TestRunnable 
    {
        ...
        public void runTest() throws Throwable 
        {
			...
			for (int i=0; i < JaxerHtmlTestUtil.getRequestCount(); i++)
			{
			    ...
			    assertEquals( "<![CDATA[Test]]>", page.getTitleText() );

				// Check that the server-side JS on the page executed and dynamically set the expected value of the specified element
				HtmlElement element = page.getHtmlElementById("p");
			    assertEquals( "PASS", element.asText());
			}
        }
    }
