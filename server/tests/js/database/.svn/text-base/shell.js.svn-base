...
const JX_MYSQL50_CID		= "@aptana.com/jxMySQL50;1"
const JX_I_MYSQL50       	= Components.interfaces.jxIMySQL50;

eval('const jxC 			= Components');
eval('const jxConstructor	= Components.Constructor');
eval('const jxResults 		= jxC.results');
eval('const jxInterfaces	= jxC.interfaces');
eval('const jxClasses 		= jxC.classes');

function 
jxlibCreateInstance (aURL, aInterface) 
{
  var rv;
  try {
    rv = jxClasses[aURL].createInstance(jxInterfaces[aInterface]);
  } catch (e) { rv = e.message; }
...
/* MySQL50
   encapsulate the logic for interacting with the MySQL50 object.
   ...
   db.connect("host", "user", "pswd", "databasename");
...
function MySQL50()
{
  return jxlibCreateInstance(JX_MYSQL50_CID, JX_I_MYSQL50);
}
