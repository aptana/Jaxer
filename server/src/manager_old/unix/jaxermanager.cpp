...
#include "../../Utils/JaxerLog/logclient.h"
...
public:
    ...
    Connection(int s) : m_boundServer(0), m_requestCount(0),
...
public:
    ...
    Server(pid_t pid, int s) :
        ...
        m_nextIdle(0), m_lastSent(0), m_secondsToLive(-1),
...
private:
    ...
    bool WakeupServer()
    {
        Log(eTRACE, "Enter Server::WakeupServer pid=%d this=%d s=%d", m_pid, this, m_socket);
...
{
public:
    void Bind(unsigned port, void (*cb)(int)) {
        Log(eTRACE, "Listener:Bind port=%d L=%d", port, this);
...
static Listener g_connectionListener;

static void NewAdministrator(int s)
{
    Log(eTRACE, "in NewAdministrator s=%d", s);
...
static void NewConnection(int s)
{
    Log(eTRACE, "In NewConnection s=%d", s);
...
static void CreateNewProcess()
{
    ...
    if (pid < 0) {
        ...
        argv[i++] = "jaxer";
#endif        
        for (iter I = g_Settings.begin(); I != g_Settings.end(); ++I, ++i)
        {
...
int main(int argc, char **argv)
{
    ...
    sprintf(sFifoName, "/tmp/.jaxer.%d.fifo", g_CommandPort);
    sprintf(sLock, "%d", g_CommandPort);
    ...
    lsi.SetLogFileName(g_JaxerLogFile);
    //Create LogServer
    ...
    Log(eNOTICE, "JaxerManager " JAXER_BUILD_ID " (web %d, command %d)", g_WebPort, g_CommandPort);
    ...
    Log(eTRACE, "Bind admin port=%d", g_CommandPort);

    g_administratorListener.Bind(g_CommandPort, NewAdministrator);
    Log(eTRACE, "Bind listener port=%d", g_WebPort);
    g_connectionListener.Bind(g_WebPort, NewConnection);
