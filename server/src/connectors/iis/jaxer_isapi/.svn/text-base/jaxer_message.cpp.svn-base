...
#include <string.h>
#include <stdio.h>
#include "jaxer_protocol.h"

/**
 * See jaxer_message.h for function documentations.
 */
void jxr_msg_init(char* buf, apr_size_t *pos, char msgtype)
{
...
void jxr_msg_end(char* buf, apr_size_t *pos)
{
...
void jxr_msg_append_int16(char* buf, apr_size_t *pos, int val)
{
    buf[(*pos)++] = (unsigned char) ((val >> 8) & 0xFF);
    buf[(*pos)++] = (unsigned char) (val & 0xFF);
}

void jxr_msg_append_byte(char* buf, apr_size_t *pos, int val)
{
    buf[(*pos)++] = (unsigned char) (val & 0xFF);
}
	
void jxr_msg_append_int32(char* buf, apr_size_t *pos, int val)
{
...
void jxr_msg_append_string(char* buf, apr_size_t *pos, const char* str)
{
...
void jxr_msg_append_bytes(char* buf, apr_size_t *pos, char* b, apr_size_t off, apr_size_t numBytes)
{
...
int jxr_msg_get_int16(char* buf, apr_size_t *pos)
{
    int b1 = buf[(*pos)++] & 0xFF;
    int b2 = buf[(*pos)++] & 0xFF;
    return (b1<<8) + b2;
...
int jxr_msg_peek_int16(char* buf, apr_size_t pos) 
{
    int b1 = buf[pos] & 0xFF;
...
char jxr_msg_get_byte(char* buf, apr_size_t *pos)
{
...
char jxr_msg_peek_byte(char* buf, apr_size_t *pos)
{
    char res = (char)buf[*pos];
    return res;
}
...
apr_size_t jxr_msg_get_bytes(char* buf, apr_size_t *pos, char* dest)
{
...
apr_size_t jxr_msg_get_string(char* buf, apr_size_t *pos, char* dest)
{
...
int jxr_msg_get_int32(char* buf, apr_size_t *pos)
{
    int b1 = buf[(*pos)++] & 0xFF;
    b1 <<= 8;
    b1 |= (buf[(*pos)++] & 0xFF);
...
char jxr_msg_get_type(char* buf)
{
	Jaxer_Header *hdr = (Jaxer_Header*) buf;
	return hdr->type;
}
    
apr_size_t jxr_msg_get_length(const char*buf, apr_size_t *pos)
{
	Jaxer_Header *hdr = (Jaxer_Header*) buf;
