...
typedef struct jaxer_worker
{
	...
	//If protocol mismatch and should try with a lower version, then this is set.
	// As soon as a retry is done, this is reset to 0.  Will never be reset to 1
	// so long as my_proto != use_proto
	int retry;
