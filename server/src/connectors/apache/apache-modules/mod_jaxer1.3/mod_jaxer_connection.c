...
static apr_status_t jxr_conn_construct(void** pac, void *params, apr_pool_t *p)
{
	...
	ap_log_perror(APLOG_MARK, APLOG_DEBUG, 0, p, "mod_jaxer: jxr_conn_construct: creating a new connection for worker %s", aw->name);
...
apr_status_t jxr_conn_setup(jaxer_worker* aw)
{
	...
	ap_log_perror(APLOG_MARK, APLOG_DEBUG, 0, aw->pool, "mod_jaxer: jxr_conn_setup: creating connection pool (min=%d keep=%d max=%d) for worker %s",
