...
static apr_status_t jxr_send_document_brigade_to_jaxer(jaxer_connection *ac, out_ctx* ctx)
{
    ...
    while (1) {
	    ...
	    /* Send a message (body) to jaxer */
            while (!done) {
                write_buf_len = fread(write_buf, HUGE_STRING_LEN, 1, responseData);
fprintf(stderr, "GRRR: %d, %d\n", write_buf_len, feof(responseData));
		    ...
		    if((rv=jxr_send_message_add_header(ctx->ac, write_buf, (int)write_buf_len, BLOCKTYPE_DOCUMENT)) != APR_SUCCESS)
		    {
