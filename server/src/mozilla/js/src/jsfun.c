...
#ifdef JAXER
		if (1/*!(tc->flags & TCF_IN_FUNCTION)*/) {
			static JSAtom *jaxerAtom = NULL;
			static JSAtom *newFunctionsAtom = NULL;

			jsval jaxer;
			if (!jaxerAtom)
				jaxerAtom = js_Atomize(cx, "Jaxer", 5/*strlen("Jaxer")*/, 0);
			if (!newFunctionsAtom)
				newFunctionsAtom = js_Atomize(cx, "__newFunctions", 14/*strlen("__newFunctions")*/, 0);

			if (OBJ_GET_PROPERTY(cx, cx->globalObject, ATOM_TO_JSID(jaxerAtom), &jaxer)) {
				JSObject *jaxerObj = NULL;
				jsval symbols;
				if (JSVAL_IS_OBJECT(jaxer) && (jaxerObj = JSVAL_TO_OBJECT(jaxer)) &&
					OBJ_GET_PROPERTY(cx, jaxerObj, ATOM_TO_JSID(newFunctionsAtom), &symbols)) {
					JSObject *symbolsObj = NULL;
					jsuint symbolCount = 0;
					if (JSVAL_IS_OBJECT(symbols) && (symbolsObj = JSVAL_TO_OBJECT(symbols)) &&
							(OBJ_GET_CLASS(cx, symbolsObj) == &js_ArrayClass) &&
							js_GetLengthProperty(cx, symbolsObj, &symbolCount)) {
						if (!nullAtom) {
							jsval val = ATOM_KEY(fun->atom);
							OBJ_SET_PROPERTY(cx, symbolsObj, INT_TO_JSID(symbolCount), &val);
						}
					}
				}
			}
