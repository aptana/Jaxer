...
static
PRBool IsJaxerDocShell(nsIDocShell *pDocShell, PRBool nullIsTrue = PR_FALSE)
{
	...
	if (!docShell) {
		return nullIsTrue ? PR_TRUE : PR_FALSE;
	}
...
#ifdef JAXER
    if (!restoring && !IsJaxerDocShell(docShell)) {
#else
    if (!restoring) {
#endif /* JAXER */
...
DocumentViewerImpl::PermitUnload(PRBool *aPermitUnload)
{
  *aPermitUnload = PR_TRUE;
  ...
  nsIDocShell *docShell = window->GetDocShell();
  NS_ENSURE_TRUE(docShell, NS_ERROR_UNEXPECTED);
  if (IsJaxerDocShell(docShell, PR_TRUE)
	  || PR_TRUE /* temporary disable unload event */) {
    return NS_OK;
  }
#endif /* JAXER */
