...
PRBool IsJaxerDocShell(nsIDocShell *pDocShell)
{
	...
	NS_ENSURE_TRUE(docShell, PR_FALSE);
...
#ifdef JAXER
  static PRUint32 gcTime = MAYBE_GC_OPERATION_WEIGHT / JAXER_OPERATION_LIMIT;
  if (gcTime--) {
    MaybeGC(cx);
	gcTime = MAYBE_GC_OPERATION_WEIGHT / JAXER_OPERATION_LIMIT;
  }
#else
  MaybeGC(cx);
#endif /* JAXER */
