...
#ifdef JAXER
    nsCOMPtr<aptIBadCertHandler> bch = do_GetInterface(callbacks);
    if (bch) {
      nsCOMPtr<aptIBadCertHandler> proxy_bch;
      NS_GetProxyForObject(NS_PROXY_TO_MAIN_THREAD,
                           NS_GET_IID(aptIBadCertHandler),
                           bch,
                           NS_PROXY_SYNC,
                           getter_AddRefs(proxy_bch));
      if (proxy_bch) {
        nsIInterfaceRequestor *csi = static_cast<nsIInterfaceRequestor*>(infoObject);
        PRBool ignoreProblem = PR_FALSE;
        rv = proxy_bch->HandleCertProblem(csi, status, hostWithPortString, 
                                          &ignoreProblem);
        if (NS_SUCCEEDED(rv) && ignoreProblem) {
          return SECSuccess;
        }
      }
    }
