                                 ...
                                 PRBool aCompileEventHandlers)
{
  ...
  if (aDocument) {
    ...
    nsCOMPtr<nsIDOMElement> aDOMElement(do_QueryInterface(this));
    nsAutoString tagname;
    aDOMElement->GetTagName(tagname);

    PRBool doThisTag = PR_FALSE;
    PRBool bIsScriptElement = PR_FALSE;

    // If this is a script tag, we check to see if it has a runat attribute
    // to see if we should fire the event
    // If this is a script tag, fire the event only if it has a runat attribute
    if(tagname.LowerCaseEqualsLiteral("script"))
    {
      bIsScriptElement = PR_TRUE;
      PRUint32 nAttri = mAttrsAndChildren.AttrCount();
      for (PRUint32 i=0; i<nAttri; i++)
      {
          nsAutoString aStr;
          const nsAttrName* aName = mAttrsAndChildren.AttrNameAt(i);
          aName->GetQualifiedName(aStr);
          for (PRUint32 j=0; sScriptRunAtTags[j] != 0; j++)
          {
              if (aStr.EqualsASCII(sScriptRunAtTags[j]))
              {
                  doThisTag = PR_TRUE;
                  break;
              }
    ...
    {
        // Get a reference to the parser service.
        nsIParserService* parserService = nsContentUtils::GetParserService();

        // If the preferences indicate we should fire events for this type of element...
        doThisTag = parserService->IsTagMonitored(parserService->HTMLAtomTagToId(this->Tag()));
    }
    ...
    if (g_doNextTag || doThisTag) {
      ...
      // Attach the element to the event
      rv = nhe->SetElement(aDOMElement);
      // This is after it has been added to DOM
      rv = nhe->SetAfterAddedToDOM(PR_TRUE);

      nsCOMPtr<nsIDOMDocument> aDOMDocument(do_QueryInterface(aDocument));
      nhe->SetDOM(aDOMDocument);

      // Fire the NewHTMLElement event out to all observers
      rv = GetEventTypeManager()->FireEvent(nhe, aptEventName_NewHTMLElement);

      if (NS_SUCCEEDED(rv)) {
        // Obtain the bind/no-bind indicator set by the event handler
        PRBool aEnableBind = PR_TRUE;
        rv = nhe->GetEnableBind(&aEnableBind);
        // If event handler has dis-allowed this element to proceed into the DOM, eradicate it.
        if (NS_SUCCEEDED(rv) && !aEnableBind) {
            aParent->RemoveChildAt(aParent->GetChildCount() - 1, PR_FALSE);
        }
        ...
        if (bIsScriptElement) {
            ...
            if (!aEnableScriptEval) {
				...
				NS_ASSERTION(sele, "Not really a script tag?");
                sele->PreventExecution();
            }
        }
      }
    } else if (bIsScriptElement) {
		/* doThisTag is false => prevent this script execution */
		nsCOMPtr<nsIScriptElement> sele = do_QueryInterface(this);
		NS_ASSERTION(sele, "Not really a script tag?");
		sele->PreventExecution();
	}
...
#ifdef JAXER
		PRBool skipScriptEvent = PR_FALSE;
...
nsGenericHTMLFrameElement::LoadSrc()
{
  ...
  return rv;
#else
  return NS_OK; // No frame loading on server.
#endif /* JAXER */
}
