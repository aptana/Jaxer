...
#ifdef JAXER
class BadCertHandler : public aptIBadCertHandler
{
public:
  BadCertHandler(nsXMLHttpRequest *request);
  virtual ~BadCertHandler();
...
private:
  nsXMLHttpRequest* mRequest;
};

NS_IMPL_THREADSAFE_ISUPPORTS1(BadCertHandler, aptIBadCertHandler)

BadCertHandler::BadCertHandler(nsXMLHttpRequest *request)
  : mRequest(request)
{
}
...
NS_IMETHODIMP
BadCertHandler::HandleCertProblem(nsIInterfaceRequestor *socketInfo, nsISSLStatus *status, const nsACString & targetSite, PRBool *_retval)
{
  return mRequest->HandleCertProblem(socketInfo, status, targetSite, _retval);
}

nsresult
nsXMLHttpRequest::HandleCertProblem(nsIInterfaceRequestor *socketInfo, nsISSLStatus *status, const nsACString & targetSite, PRBool *_retval)
{
...
NS_IMETHODIMP
nsXMLHttpRequest::GetOnsslcerterror(aptIBadCertHandler * *aOnsslcerterror)
{
...
NS_IMETHODIMP
nsXMLHttpRequest::SetOnsslcerterror(aptIBadCertHandler * aOnsslcerterror)
{
...
#ifdef JAXER
/*
 * This piece is pretty much the same as ParserWriteFunc() in nsParser.cpp.
 * We try to detect the charset from the document.
 * The limitation is in DetectMetaTag that for speeddy reasons, it only
 * goes upto 2K.  If the meta tag is not within 2k, then we will not
 * be able to find it.
 */
nsresult
nsXMLHttpRequest::DetectCharsetFromDoc(nsACString& aCharset)
{
  aCharset.Truncate();
  nsresult rv;

  nsCAutoString contentTypeValue;
  nsCOMPtr<nsIChannel> channel(do_QueryInterface(mReadRequest));
  if (!channel) {
    channel = mChannel;
    if (!channel) {
  ...
  // This is only for HTML page, if it is not, then do not bother.
  rv = channel->GetContentType(contentTypeValue);
  if (NS_FAILED(rv) || !contentTypeValue.Equals("text/html"))
      return NS_ERROR_NOT_AVAILABLE;

  PRInt32 dataLen = mResponseBody.Length();
  const char * buf = mResponseBody.get();
  PRInt32 guessSource;
  ...
  if (ps.DetectMetaTag(buf, dataLen, guess, guessSource) ||
        ((dataLen >= 4) &&
         ps.DetectCharsetByteOrderMark(buf, dataLen, guess, guessSource))) {
      nsCOMPtr<nsICharsetAlias> alias(do_GetService(NS_CHARSETALIAS_CONTRACTID));
      rv = alias->GetPreferred(guess, preferred);
...
nsXMLHttpRequest::GetInterface(const nsIID & aIID, void **aResult)
{
  ...
  if (aIID.Equals(NS_GET_IID(aptIBadCertHandler)) && mOnSSLCertErrorListener) {
    *aResult = new BadCertHandler(this);
