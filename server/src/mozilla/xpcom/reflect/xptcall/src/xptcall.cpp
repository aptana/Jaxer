...
nsXPTCStubBase::QueryInterface(REFNSIID aIID,
                               void **aInstancePtr)
{
#ifdef JAXER_XPTCALL_WEBSERVICES_FIX
    if (aIID.Equals(mIID)) {
#else
    if (aIID.Equals(mEntry->mIID)) {
#endif /* JAXER_XPTCALL_WEBSERVICES_FIX */
        NS_ADDREF_THIS();
                  ...
                  nsISomeInterface* *aResult)
{
    ...
    nsresult rv = EntryToInfo(iie, &aInterfaceInfo);
    if(NS_FAILED(rv))
        return rv;
    nsXPTCStubBase* newbase = new nsXPTCStubBase(aOuter, aIID, aInterfaceInfo);
#else
...
#ifdef JAXER_XPTCALL_WEBSERVICES_FIX
EXPORT_XPCOM_API(nsresult)
NS_GetXPTCallStubII(nsIInterfaceInfo* aInterfaceInfo, nsIXPTCProxy* aOuter,
                  nsISomeInterface* *aResult)
{
    NS_ENSURE_ARG(aOuter && aResult);

    const nsIID* aIID;
    aInterfaceInfo->GetIIDShared(&aIID);
    nsXPTCStubBase* newbase = new nsXPTCStubBase(aOuter, *aIID, aInterfaceInfo);
    if (!newbase)
        return NS_ERROR_OUT_OF_MEMORY;
