...
bool Connection::RequestArrived()
    {
        ...
        {
            // WEB REQUEST
			if (!g_ConnectionSettings[eWEB_HTTP_PROTOCOL].mEnabled && m_reqType != eNEW_ADMIN_REQUEST_MSG)
			{
				Log(eWARN, "RequestArrived: HTTP request without enabling internal webserver");
				return false;
			}
            Log(eDEBUG, "Connection::RequestArrived: WEB REQUEST");
			if (m_reqType != eNEW_ADMIN_REQUEST_MSG)
				m_reqType = eNEW_HTTP_REQUEST_MSG;
        }else
#endif
		if (buf[0] == bt_DebugRequest)
        {
			...
			if (m_reqType == eNEW_ADMIN_REQUEST_MSG)
			{
				Log(eERROR, "Connection::RequestArrived: got debug msg on admin request/connection.");
                return false;
			}
#endif
            // get the msg
